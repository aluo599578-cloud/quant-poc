"""
double_ma_trend_filter_lean.py — 双均线 + MA200 趋势过滤
=======================================================
...
class DoubleMaTrendFilterLean(CtaTemplate):
    """双均线 + MA200 趋势过滤 (Lean 风格)"""
    ...
    fixed_size: int = 1
    warmup_bars: int = 250  # 200 + buffer

    fast_ma0: float = 0.0
    slow_ma0: float = 0.0
    bar_count: int = 0

    parameters = ["fast_window", "slow_window", "trend_window", "fixed_size", "warmup_bars"]
    variables = ["fast_ma0", "slow_ma0", "bar_count"]

    def on_init(self):
        self.write_log("双均线+MA200 Lean 策略初始化")
